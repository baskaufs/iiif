...
A viewer that supports annotations provides some mechanism to allow the user to move through the annotations to learn more about the details of the image depicted on the canvas. Each project page has a link that displays the manifest in the Annona viewer, which zooms in on each annotation of a canvas as you click the "next annotation" arrow button.

The standard viewers have a limited ability to control the display of annotations across canvases. For that reason, we are using a presentation system called *Exhibit* that will load a IIIF manifest, then allow your to zoom in on and annotate regions of canvases in any order to create a presentation. We have set our presentations so that the annotations cycle when you scroll down through the exhabit frame.
